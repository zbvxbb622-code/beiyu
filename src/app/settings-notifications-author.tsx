...
export default function SettingsNotificationsAuthorScreen() {
  const router = useRouter();
  const [authorUpdate, setAuthorUpdate] = useState(true);
      ...
      >
        <View style={styles.header}>
          <Pressable
            ...
            testID="settings-notifications-author-back-button"
          >
            <View style={styles.headerSideInner}>
              <ChevronLeft color={colors.text} size={26} />
            </View>
          </Pressable>
          <Text style={styles.headerTitle}>关注作者的更新</Text>
        ...
        <View style={styles.body}>
          <View style={styles.card}>
            <Row
              title="关注作者的更新"
              onPress={() => setAuthorUpdate((v) => !v)}
              trailing={<Toggle value={authorUpdate} onColor={colors.red} />}
              testID="author-update"
            />
            <Row
              title="关注未读提醒"
              onPress={() => setUnreadReminder((v) => !v)}
              trailing={<Toggle value={unreadReminder} onColor={colors.red} />}
              isLast
              testID="author-unread-reminder"
            />
          </View>
        </View>
      </ScrollView>
